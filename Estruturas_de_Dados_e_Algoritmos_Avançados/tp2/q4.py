class Trie(object):
    ...
    def autocorrigir(self, string):
        """Sugere correções para uma string incorreta baseada no prefixo mais longo"""
        no_atual = self.__raiz
        prefixo = ""
        for c in string:
            if c in no_atual.filhos:
                no_atual = no_atual.filhos[c]
                prefixo += c
            else:
                break
        if prefixo == string and "*" in no_atual.filhos:
            return [string]
        return self.__recupera_palavras(no_atual, prefixo)
...
print(trie.autocorrigir("moçs"))  # ['moço', 'moça']
print(trie.autocorrigir("moxasfdij"))  # ['moço', 'moça', 'modo']
print(trie.autocorrigir("moça"))  # ['moça']
