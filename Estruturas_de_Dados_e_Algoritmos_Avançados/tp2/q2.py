class Trie(object):
    ...
    def imprime_chaves(self):
        """Imprime todas as chaves na trie"""

        def _imprime_chaves(no):
            for chave, filho in no.filhos.items():
                print(chave)
                if filho:
                    _imprime_chaves(filho)

        _imprime_chaves(self.__raiz)
...
print('\nImprimindo todas as chaves na trie:')
t.imprime_chaves()